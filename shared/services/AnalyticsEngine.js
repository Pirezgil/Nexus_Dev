...
class AnalyticsEngine {
     ...
     */
    async calculateCRMStats(companyId, period) {
        try {
            const now = new Date();
            const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
            const startOfLastMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);
            const endOfLastMonth = new Date(now.getFullYear(), now.getMonth(), 0);
            logger_1.logger.info(`Calculating CRM stats for company: ${companyId}`);
            ...
            const totalCustomers = this.calculateKPIWithTrend(currentStats.totalCustomers, previousStats.totalCustomers);
            const activeCustomers = this.calculateKPIWithTrend(currentStats.activeCustomers, previousStats.activeCustomers);
            const newCustomersThisMonth = this.calculateKPIWithTrend(currentStats.newCustomers, previousStats.newCustomers);
            const averageInteractions = this.calculateKPIWithTrend(currentStats.averageInteractions, previousStats.averageInteractions);
            return {
                totalCustomers,
                activeCustomers,
            ...
            logger_1.logger.error('Error calculating CRM stats', { error, companyId });
            throw error;
        }
    }
    /**
     ...
     */
    async getCRMPeriodStats(companyId, period) {
     ...
     */
    async calculateServicesStats(companyId, period) {
        try {
            const now = new Date();
            const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
            const startOfLastMonth = new Date(now.getFullYear(), now.getMonth() - 1, 1);
            const endOfLastMonth = new Date(now.getFullYear(), now.getMonth(), 0);
            logger_1.logger.info(`Calculating Services stats for company: ${companyId}`);
            // Current period
            ...
            // Calculate KPIs
            const totalRevenue = this.calculateKPIWithTrend(currentStats.totalRevenue, previousStats.totalRevenue);
            const completedAppointments = this.calculateKPIWithTrend(currentStats.completedAppointments, previousStats.completedAppointments);
            const averageTicket = this.calculateKPIWithTrend(currentStats.averageTicket, previousStats.averageTicket);
            return {
                totalRevenue,
            ...
            logger_1.logger.error('Error calculating Services stats', { error, companyId });
            throw error;
        }
    }
    /**
     ...
     */
    async getServicesPeriodStats(companyId, period) {
     ...
     */
    async calculateAgendamentoStats(companyId, period) {
        try {
            const now = new Date();
            const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
            const tomorrow = new Date(today.getTime() + 24 * 60 * 60 * 1000);
            logger_1.logger.info(`Calculating Agendamento stats for company: ${companyId}`);
            ...
            logger_1.logger.error('Error calculating Agendamento stats', { error, companyId });
            throw error;
        }
    }
    // ===============================
     ...
     */
    calculateKPIWithTrend(current, previous) {
     ...
     */
    async calculateConversionRate(companyId, period) {
     ...
     */
    async getCustomersBySource(companyId, period) {
     ...
     */
    async getInteractionsByType(companyId, period) {
     ...
     */
    async getCustomerGrowthChart(companyId) {
        const days = 30;
        const result = [];
     ...
     */
    async getTopServices(companyId, period) {
        ...
        return Promise.all(topServices.map(async (service) => {
     ...
     */
    async getProfessionalsPerformance(companyId, period) {
     ...
     */
    async getRevenueByDay(companyId, period) {
        const days = 30;
        ...
        for (let i = 0; i < days; i++) {
            ...
            const dayRevenue = dailyRevenue.find(d => d.completedAt.toISOString().split('T')[0] === dateStr);
            result.push({
                date: dateStr,
     ...
     */
    async getPaymentStatusDistribution(companyId, period) {
     ...
     */
    async calculateBookingEfficiency(companyId) {
     ...
     */
    async getAppointmentsByStatus(companyId) {
